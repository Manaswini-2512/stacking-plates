...
"""lass EmojiParticle:
    def __init__(self):
        ...
        self.y = HEIGHT + random.randint(0, HEIGHT // 2)
        self.speed = random.uniform(0.5, 1.5)
        self.font = pygame.font.SysFont("Segoe UI Emoji", random.randint(36, 60))
        self.wave_offset = random.uniform(0, 2 * math.pi)

    def update(self):
        self.y -= self.speed
        self.x += math.sin(pygame.time.get_ticks() * 0.002 + self.wave_offset) * 0.5
        if self.y < -60:
            self.__init__()

    def draw(self, surface):
        emoji_surface = self.font.render(self.emoji, True, (0, 0, 0))
        surface.blit(emoji_surface, (int(self.x), int(self.y)))

emoji_particles = [EmojiParticle() for _ in range(20)]"""
...
class ConfettiParticle:
    def __init__(self):
        ...
        self.color = random.choice([(255, 0, 0), (0, 255, 0), (0, 150, 255), (255, 215, 0), (255, 105, 180)])
...
confetti_particles = [ConfettiParticle() for _ in range(80)]
#emoji_particles = [EmojiParticle() for _ in range(20)]
...
def draw_screens():
    ...
    if current_screen == "name_prompt":
        ...
        # Optional: fill background or use an image
        ...
        # Fonts and colors
        headline_font = pygame.font.SysFont("Lucida Handwriting", 84, bold=True)
        sub_font = pygame.font.SysFont("Segoe UI", 44, bold=True)
        shadow_color = (60, 60, 60)
        main_color = (255, 215, 0)

        # 🎉 CONGRATULATIONS! with shadow
        congrats_text = " CONGRATULATIONS!"
        shadow = headline_font.render(congrats_text, True, shadow_color)
        main = headline_font.render(congrats_text, True, main_color)
        screen.blit(shadow, (center_x - shadow.get_width() // 2 + 4, center_y - 180 + 4))
        screen.blit(main, (center_x - main.get_width() // 2, center_y - 180))

        # Level completed text
        lvl_text = sub_font.render(f"Level {selected_level+1} Completed", True, BLACK)
        screen.blit(lvl_text, (center_x - lvl_text.get_width() // 2, center_y - 80))

        # Moves and time
        mt_text = sub_font.render(f"Moves: {score} | Time: {elapsed_time}s", True, BLACK)
        screen.blit(mt_text, (center_x - mt_text.get_width() // 2, center_y - 20))

        # Buttons centered at bottom
        win_back_button.rect.topleft = (center_x - 230, center_y + 80)
        win_next_button.rect.topleft = (center_x + 50, center_y + 80)
       ...
       timeout_font_large = pygame.font.SysFont("Lucida Handwriting", 80, bold=True)
       timeout_font_medium = pygame.font.SysFont("Comic Sans MS", 40, bold=True)

       draw_text_center("⏰ TIME'S UP!", timeout_font_large, RED, CONTENT_RECT.top + 60)
       draw_text_center(f"Level {selected_level+1} failed", timeout_font_medium, BLACK, CONTENT_RECT.top + 160)
       draw_text_center(f"Moves: {score} | Time: {elapsed_time}s", timeout_font_medium, BLACK, CONTENT_RECT.top + 220)
